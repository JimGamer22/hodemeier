...
def render_predictor_tab():
    # Wir nutzen header statt title, damit es in der Tab-Struktur besser aussieht
    ...
    with tab1:
        ...
        yes_pct = st.slider("Wie viel % steht bei JA?", 1, 99, 50, key="yes_slider")

        if st.button("Chance berechnen", key="calc_btn"):
    ...
    with tab3:
        st.subheader("Daten Historie")
        if not df.empty:
            st.dataframe(df.sort_values(by='runde', ascending=False).head(15), use_container_width=True)
        else:
